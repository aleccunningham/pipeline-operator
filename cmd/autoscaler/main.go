...
import (
	...
	"github.com/marjoram/pipeline-operator/operator/autoscaler"
)
...
func (m *Main) Run(stopC <-chan struct{}) error {
	...
	op, err := operator.New(m.config, scaleCli, crdCli, k8sCli, m.logger)
	if err != nil {
		return err
	}
...
func (m *Main) getKubernetesClients() (autoscalerdukev1alpha1.Interface, crd.Interface, kubernetes.Interface, error) {
	var err error
	var cfg *rest.Config
	...
	pipeCli, err := autoscalerdukev1alpha1.NewForConfig(cfg)
	if err != nil {
		return nil, nil, nil, err
	}
